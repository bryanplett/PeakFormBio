...
function CoachAdmin({ clients, activeId, onSelect, sessions, plan, athlete }) {
  const [localId, setLocalId] = useStateC(activeId || (clients[0] && clients[0].id) || 'ava');
  const clientId = (activeId != null) ? activeId : localId;
  const setClientId = (id) => { onSelect ? onSelect(id) : setLocalId(id); };
  const [selSession, setSelSession] = useStateC(null);

  const data = sessions || [];
  const hasData = data.length > 0;
  const sorted = [...data].sort((a, b) => new Date(b.date) - new Date(a.date));
  const client = clients.find(c => c.id === clientId) || clients[0] || { name: '—' };
  ...
  const planDays = ((plan && plan.plan_data) || []).map(d => ({
          ...
          return (
            <button key={c.id} onClick={() => { setClientId(c.id); setSelSession(null); }} style={{
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{c.status || (c.id === clientId ? 'Active' : '')}</div>
              </div>
            </button>
          );
        })}
      ...
      <div style={{ flex: 1, minWidth: 0, padding: '16px 22px 40px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18, flexWrap: 'wrap' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>{hasData && plan ? `${plan.goal || plan.title || 'Plan'} · ${plan.title || ''}` : 'No plan assigned'}</div>
          </div>
        ...
        {!hasData ? (
          <CCard style={{ padding: 50, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13.5, color: 'rgba(255,255,255,0.5)' }}>{client.name} hasn't started tracking yet. Assign a plan to get them going.</div>
          </CCard>
        ) : (
          <>
            {/* KPIs */}
